...
export function frequencyToRowIndex(
	...
	const f = Math.min(Math.max(frequency, minFrequency), maxFrequency);
	const linear = (f - minFrequency) / (maxFrequency - minFrequency);
	const logarithmic =
		Math.log(f / minFrequency) / Math.log(maxFrequency / minFrequency);
	const pos = linear + clampLogAmount(logAmount) * (logarithmic - linear);
